...
BEGIN
    ...
    IF length(i_password) >= 8 AND ((i_email IS NOT NULL AND i_email != '') OR (i_balance IS NOT NULL AND i_balance != ''))
    THEN 
        ...
        IF (i_balance IS NOT NULL AND i_balance != '') AND i_balance >= 0 AND (i_email IS NULL OR i_email = '')
			THEN
			INSERT INTO cs4400spring2020.Customer(username,balance,stationName)
			VALUES(i_username,i_balance,NULL);
            INSERT INTO login_classifier(username,userType) 
            VALUES (i_username,'Customer');
		END IF;
        IF (i_email IS NOT NULL AND i_email != '') AND (i_balance IS NULL OR i_balance = '')
			THEN
			INSERT INTO cs4400spring2020.Employee(username,email) 
			VALUES(i_username,i_email);
            IF i_type = 'Admin'
				THEN
                INSERT INTO cs4400spring2020.`Admin`(username)
                VALUES(i_username);
                INSERT INTO login_classifier(username,userType) 
				VALUES (i_username,'Admin');
			END IF;
            IF i_type = 'Manager'
				THEN
                INSERT INTO cs4400spring2020.Manager(username)
                VALUES(i_username);
                INSERT INTO login_classifier(username,userType) 
            ...
            IF i_type = 'Staff'
				THEN 
                INSERT INTO cs4400spring2020.Staff(username)
                VALUES(i_username);
                INSERT INTO login_classifier(username,userType) 
				VALUES (i_username,'Staff');
			END IF;
		END IF;
        IF (i_balance IS NOT NULL AND i_balance != '') AND i_balance >= 0 AND (i_email IS NOT NULL AND i_email != '')
			THEN
			INSERT INTO cs4400spring2020.Customer(username,balance,stationName)
			VALUES(i_username,i_balance,NULL);
            			INSERT INTO cs4400spring2020.Employee(username,email) 
            ...
            IF i_type = 'Admin'
                ...
                VALUES(i_username);
                INSERT INTO login_classifier(username,userType) 
				VALUES (i_username,'Admin-Customer');
			END IF;
            IF i_type = 'Manager'
				THEN
                INSERT INTO cs4400spring2020.Manager(username)
                VALUES(i_username);
                INSERT INTO login_classifier(username,userType) 
				VALUES (i_username,'Manager-Customer');
			END IF;
            IF i_type = 'Staff'
				THEN 
                INSERT INTO cs4400spring2020.Staff(username)
                VALUES(i_username);
                INSERT INTO login_classifier(username,userType) 
				VALUES (i_username,'Staff-Customer');
			END IF;
        END IF;
    END IF;
...
BEGIN
	...
	INSERT INTO mn_filter_foodTruck_result(foodTruckName,stationName,remainingCapacity,staffCount,menuItemCount)
	SELECT FoodTruck.foodTruckName, FoodTruck.stationName, (Station.capacity - (SELECT COUNT(foodTruckName) AS capacity FROM FoodTruck WHERE stationName = Station.stationName)), COUNT(DISTINCT Staff.username), COUNT(DISTINCT MenuItem.foodName)
    FROM FoodTruck
    INNER JOIN Station
    ...
    (i_foodTruckName = FoodTruck.foodTruckName OR i_foodTruckName = "" OR i_foodTruckName IS NULL) AND
    (i_stationName = FoodTruck.stationName OR i_stationName = "" OR i_stationName IS NULL) AND
    ((i_hasRemainingCapacity = TRUE AND (Station.capacity - (SELECT COUNT(foodTruckName) AS capacity FROM FoodTruck WHERE stationName = Station.stationName))>0) OR (i_hasRemainingCapacity = FALSE))
    GROUP BY foodTruckName
    HAVING
    ((i_minStaffCount IS NULL AND i_maxStaffCount IS NULL) OR (i_minStaffCount IS NULL AND COUNT(DISTINCT Staff.username) <= i_maxStaffCount) OR (i_maxStaffCount IS NULL AND i_minStaffCount <= COUNT(DISTINCT Staff.username)) OR (COUNT(DISTINCT Staff.username) BETWEEN i_minStaffCount AND i_maxStaffCount));
...
WHERE username = i_staffName AND foodTruckName = NULL;
...
BEGIN
	...
	SELECT FoodTruck.foodTruckName AS foodTruckName, FoodTruck.stationName AS stationName, MenuItem.foodName AS foodName, MenuItem.price AS price
	FROM FoodTruck
	INNER JOIN MenuItem
	ON FoodTruck.foodTruckName = MenuItem.foodTruckName
	WHERE
...
BEGIN
	...
	SELECT Customer.stationName AS stationName, Station.buildingName AS buildingName, GROUP_CONCAT(DISTINCT(BuildingTag.tag) SEPARATOR ',') AS tags, Building.`description`,Customer.balance AS balance
	FROM Customer 
    INNER JOIN Station ON Customer.stationName = Station.stationName
    INNER JOIN Building ON Station.buildingName = Building.buildingName
...
BEGIN
    ...
    SELECT FoodTruck.foodTruckName AS foodTruckName, concat(`User`.firstName, ' ', `User`.lastName) AS managerName, GROUP_CONCAT(DISTINCT(MenuItem.foodName) SEPARATOR ',') AS foodNames
    FROM FoodTruck
    INNER JOIN `User` ON `User`.username = FoodTruck.managerUsername
    INNER JOIN MenuItem ON MenuItem.FoodTruckName = FoodTruck.foodTruckName
    INNER JOIN Customer ON Customer.stationName = FoodTruck.stationName
    WHERE Customer.username = i_customerUsername
    GROUP BY FoodTruck.foodTruckName;
...
BEGIN
    ...
    INSERT INTO cs4400spring2020.Orders(customerUsername,date)
...
BEGIN
	...
	IF (SELECT balance FROM Customer WHERE Customer.username = (SELECT DISTINCT customerUsername FROM Orders WHERE orderID = i_orderId)) - (SELECT i_purchaseQuantity * price AS temp2 FROM MenuItem WHERE MenuItem.foodName = i_foodName AND MenuItem.foodTruckName = i_foodTruckName) >= 0
    THEN
    INSERT INTO cs4400spring2020.OrderDetail(orderID,foodTruckName,foodName,purchaseQuantity)
    VALUES (i_orderId,i_foodTruckName,i_foodName,i_purchaseQuantity);
    UPDATE Customer
    SET balance = balance - (SELECT i_purchaseQuantity * price AS temp2 FROM MenuItem WHERE MenuItem.foodName = i_foodName AND MenuItem.foodTruckName = i_foodTruckName)
    WHERE username = (SELECT DISTINCT customerUsername FROM Orders WHERE orderID = i_orderId);
    END IF;
...
BEGIN
    ...
    SELECT Orders.`date`,Orders.orderID,SUM(OrderDetail.purchaseQuantity * MenuItem.price) AS orderTotal,GROUP_CONCAT(DISTINCT(OrderDetail.foodName) SEPARATOR ',') AS foodNames,SUM(OrderDetail.purchaseQuantity) AS foodQuantity
    FROM Orders
    LEFT JOIN OrderDetail ON Orders.orderID = OrderDetail.orderID                          
    LEFT JOIN MenuItem ON (OrderDetail.foodName = MenuItem.foodName AND OrderDetail.foodTruckName = MenuItem.foodTruckName) 
    WHERE Orders.customerUsername = i_customerUsername
